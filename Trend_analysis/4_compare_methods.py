...
for city in all_cities['CITY'].unique():
    ...
    original_std = city_data['TEMP'].std()
    
    rolling_30d_std = city_data['TEMP'].rolling(window=30, center=True).mean().std()
...
comparison_df.to_csv('Trend_analysis/results/method_comparison.csv', index=False)
print("\nResults saved to Trend_analysis/results/method_comparison.csv")
