...
for idx, city in enumerate(all_cities['CITY'].unique()):
    ...
    for w in windows:
        city_data[f'rolling_{w}d'] = city_data['TEMP'].rolling(window=w, center=True).mean()
        print(f"{city} - {w} day window: STD reduced by {((city_data['TEMP'].std() - city_data[f'rolling_{w}d'].std())/city_data['TEMP'].std()*100):.1f}%")
    
    axes[idx].plot(city_data['DATE'], city_data['TEMP'], alpha=0.3, label='Original')
...
plt.tight_layout()
plt.savefig('Trend_analysis/results/rolling_average.png')
plt.show()
